...
const STATS = [
  { icon: Trophy,    value: 120000, suffix: "K+", label: "Matches Played",       format: (n: number) => `${Math.round(n / 1000)}K+` },
  { icon: Calendar,  value: 12000,  suffix: "K+", label: "Tournaments Hosted",   format: (n: number) => `${Math.round(n / 1000)}K+` },
  { icon: Users,     value: 850000, suffix: "K+", label: "Players Registered",   format: (n: number) => `${Math.round(n / 1000)}K+` },
  { icon: Globe2,    value: 40,     suffix: "+",  label: "Gaming Communities",   format: (n: number) => `${Math.round(n)}+` },
  { icon: Gamepad2,  value: 18,     suffix: "",   label: "Supported Games",      format: (n: number) => `${Math.round(n)}` },
];
